...
# Brazil ATMs/100,000 adults
atmb <- subset(wdifin10, country=="Brazil", select = c(year, FB.ATM.TOTL.P5) ) 
atmb <- na.omit(atmb)
atmb.hat <- predict( lm( log(FB.ATM.TOTL.P5) ~ year, data=atmb))
qplot(year, log(FB.ATM.TOTL.P5), data=atmb, geom="step")
last_plot() + geom_line(aes(x=year, y=atmb.hat), col=2)
atmb.g <-coefficients( lm( log(FB.ATM.TOTL.P5) ~ year, data=atmb) )[2]
atmb.g

# China ATMs/100,000 adults
atmc <- subset(wdifin10, country=="China", select = c(year, FB.ATM.TOTL.P5) ) 
atmc <- na.omit(atmc)
atmc.hat <- predict( lm( log(FB.ATM.TOTL.P5) ~ year, data=atmc))
qplot(year, log(FB.ATM.TOTL.P5), data=atmc, geom="step")
last_plot() + geom_line(aes(x=year, y=atmc.hat), col=2)
atmc.g <-coefficients( lm( log(FB.ATM.TOTL.P5) ~ year, data=atmc) )[2]
atmc.g
...
# Bank capital to assets ratio (%)
plot(wwdifin$year, wwdifin$FB.BNK.CAPA.ZS, type="s", main="Bank capital to assets ratio (%)", sub="World, Source: WDI")

# Bank nonperforming loans to total gross loans (%)
plot(wwdifin$year, wwdifin$FB.AST.NPER.ZS, type="s", main="Bank nonperforming loans to total gross loans (%)", sub="World, Source: WDI")

# Claims on central government, etc. (% GDP)
plot(wwdifin$year, wwdifin$FS.AST.CGOV.GD.ZS, type="s", main="Claims on central government, etc. (% GDP)", sub="World, Source: WDI")

# Claims on other sectors of the domestic economy (% of GDP)
plot(wwdifin$year, wwdifin$FS.AST.DOMO.GD.ZS, type="s", main="Claims on other sectors of the domestic economy (% of GDP)", sub="World, Source: WDI")

# Domestic credit provided by financial sector (% of GDP)
plot(wwdifin$year, wwdifin$FS.AST.DOMS.GD.ZS, type="s", main="Domestic credit provided by financial sector (% of GDP)", sub="World, Source: WDI")

# Domestic credit to private sector (% of GDP)
plot(wwdifin$year, wwdifin$FS.AST.PRVT.GD.ZS, type="s", main="Domestic credit to private sector (% of GDP)", sub="World, Source: WDI")

# Domestic credit to private sector by banks (% of GDP)
plot(wwdifin$year, wwdifin$FD.AST.PRVT.GD.ZS, type="s", main="Domestic credit to private sector by banks (% of GDP)", sub="World, Source: WDI")
